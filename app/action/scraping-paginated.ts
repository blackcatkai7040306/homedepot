 ...
 */
async function scrapeUrlPaginated(
  url: string
...
function parseHomeDepotProductsNew(html: string): ProductData[] {
  ...
  try {
    ...
    productCards.each((index, element) => {
      ...
      // </h3>
      const h3Element = $productCard
        .find('h3.sui-flex-col, h3[class*="sui-line-clamp"]')
        .first()

      const brand = h3Element
        .find('span[data-testid="attribute-brandname-inline"]')
        .text()
        .trim()
      const label = h3Element
        .find('span[data-testid="attribute-product-label"]')
        .text()
        .trim()
      const title = brand && label ? `${brand} ${label}` : label || brand

      // Extract price information from data-testid="price-simple"
      const priceContainer = $productCard
        .find('div[data-testid="price-simple"]')
        .first()
      ...
      if (priceContainer.length > 0) {
        ...
        const priceMain = priceContainer
          .find(
            ".sui-font-display.sui-text-3xl, .sui-font-display.sui-text-4xl"
          )
          .text()
          .trim()
        const priceCents = priceContainer
          .find(".sui-sr-only")
          .next(".sui-font-display.sui-text-xs")
          .text()
          .trim()
        ...
        const wasPriceText = priceContainer
      ...
      let pickup = ""
      const pickupContainer = $productCard
        .find('[data-component*="FulfillmentPodStore"]')
        .first()
      ...
      let delivery = ""
      const deliveryContainer = $productCard
        .find('[data-component*="FulfillmentPodShipping"]')
        .first()
      ...
      if (index < 3) {
        console.log(`Product ${index + 1}:`, {
          ...
          delivery:
            delivery.length > 30 ? delivery.substring(0, 30) + "..." : delivery,
          hasImage: !!image,
          hasUrl: !!productUrl,
        })
      }
...
export async function scrapeHomeDepotRefrigeratorsPaginated(
  ...
  try {
    ...
    console.log(
      `✓ First page scraped: ${firstPageProducts.length} products found`
    )
    ...
    pages.push({
      ...
      nextPageUrl:
        productsPerPage > 0 ? `${baseUrl}?Nao=${productsPerPage}` : undefined,
    })
    ...
    for (const selector of paginationSelectors) {
      ...
      if (elements.length > 0) {
        console.log(
          `🔍 Found pagination elements with selector: ${selector} (${elements.length} elements)`
        )
        ...
        const numbers = elements
          .map((_, el) => {
            ...
            console.log(
              `   - Element: text="${text}", href page=${hrefPageNum}, text num=${textNum}`
            )
        ...
        if (numbers.length > 0) {
          ...
          maxPageFromPagination = Math.max(
            maxPageFromPagination,
            maxFromSelector
          )
    ...
    if (productsPerPage === 0 || maxPageFromPagination <= 1) {
      ...
      console.log(
        `✅ Only 1 page detected. Total: ${allProducts.length} products`
      )
      return {
        success: true,
        baseUrl,
    ...
    for (let page = 2; page <= maxPageFromPagination; page++) {
      ...
      console.log(
        `✓ Page ${page} scraped: ${pageProducts.length} products found`
      )

      // Detect if this is the last page
      const isLastPage =
        pageProducts.length < productsPerPage || pageProducts.length === 0
      ...
      if (pageProducts.length > 0) {
        pages.push({
          ...
          nextPageUrl: isLastPage
            ? undefined
            : `${baseUrl}?Nao=${page * productsPerPage}`,
        })
      ...
      if (isLastPage) {
        console.log(
          `📄 Reached last page: ${page} (found ${pageProducts.length} products)`
        )
        break
      }
    }
    ...
    console.log(
      `   - Average products per page: ${Math.round(
        allProducts.length / pages.length
      )}`
    )
